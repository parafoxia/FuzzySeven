# Fuzzy - The iron-booted robotic resident of The Tsunami Zone.
...
from fuzzy.utils import chron, string


class ErrorHandler:
    async def handle_error(self, err: str, ctx: commands.Context, exc: Exception) -> None:
        async with ctx.bot.session.post("https://mystb.in/documents", data=traceback.format_exc()) as response:
            if response.status == 200:
                data = await response.json()
                hub = ctx.bot.get_cog("Hub")
                await hub.stdout.send(f"Something went wrong: <https://mystb.in/{data['key']}>")

        if err == "on_command_error":
            await ctx.send(
                "Something went wrong. An automatic error report was submitted, but you should report it to Parafoxia."
            )
    ...
    async def handle_command_error(self, ctx: commands.Context, exc: Exception) -> None:
        ...
        if isinstance(exc, commands.NotOwner):
            return await ctx.send("Only Parafoxia can do that.")
